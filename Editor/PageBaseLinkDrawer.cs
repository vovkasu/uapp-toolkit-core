...
namespace Assets.Editor
{
    ...
    public class PageBaseLinkDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            {
                var entryPointBase = Object.FindObjectOfType<EntryPointBase>();
                if (entryPointBase != null)
                {
                    var navigationControllerBase = entryPointBase.NavigationControllerBase;
                    if (navigationControllerBase != null && navigationControllerBase.PageLinkList.Count > 0)
                    {
                        var titles = navigationControllerBase.PageLinkList.Select(_ => _.SceneTitle).ToArray();
                        ...
                        if (property.propertyPath.Contains(".Array.data["))
                        {
                            ...
                            if (element.Contains("["))
                            {
                                ...
                                index = System.Convert.ToInt32(element.Substring(element.IndexOf("[")).Replace("[", "")
                                    .Replace("]", ""));
                        ...
                        var oldPageLink =
                            navigationControllerBase.PageLinkList.FirstOrDefault(_ =>
                                _.SceneGuid == targetPageBaseLink.SceneGuid);
                        var indexOf = navigationControllerBase.PageLinkList.IndexOf(oldPageLink);
                        ...
                        if (titles.Length > 0)
                        {
                            var newPageLink = navigationControllerBase.PageLinkList[newIndex];
